...
class BarIndicator(View):
    ...
    def draw(self):
        ...
        if self.is_vertical:
            ...
            main_lines = []
            main_lines.append(
                [Tixel('=', Color.White, Color.Black), Tixel('=', Color.White, Color.Black)])
            main_lines += [[Tixel(' ', Color.White, Color.Black), Tixel(' ', Color.White, Color.Black)]
            ...
            if should_show_readout:
                ro = [Tixel(c, Color.White, Color.Black) for c in readout]
            ...
            for line in lines:
                ...
                centered_lines.append(left_fill * [Tixel(' ', Color.White, Color.Black)] +
                                      line + right_fill * [Tixel(' ', Color.White, Color.Black)])
            ...
            if self.readout_position is None:
                ...
                main_line = []
                main_line.append(Tixel('[', Color.White, Color.Black))
                ...
                if should_show_readout:
                    if self.readout_position == 'beginning':
                        readout = readout[:self.readout_size].ljust(
                            self.readout_size, ' ')
                        line = [Tixel(c, Color.White, Color.Black) for c in readout] + \
                            [Tixel(' ', Color.White, Color.Black)] + main_line
                    else:
                        readout = readout[:self.readout_size].rjust(
                            self.readout_size, ' ')
                        line = main_line + \
                            [Tixel(' ', Color.White, Color.Black)] + \
                            [Tixel(c, Color.White, Color.Black)
                             for c in readout]
                else:
                    line = main_line
